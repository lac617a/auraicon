...
async function build(rootDir: string) {
  try {
    ...
    const distAuraiconsDir = join(distDir, 'auraicon');
    const distSvgDir = join(distDir, 'svg');

    await Promise.all([fs.emptyDir(iconDir), fs.emptyDir(distDir)]);
    await fs.emptyDir(distSvgDir), await fs.emptyDir(distAuraiconsDir);

    const pkgData = await fs.readJson(pkgJsonPath);
    const version = pkgData.version as string;

    const srcSvgData = await getSvgs(srcSvgDir, distSvgDir, distAuraiconsDir);
            ...
            if (item.isElem()) {
              item.eachAttr((attr) => {
                if (attr.name === 'fill') {
                  ...
                  item.class.add('aura-stroke-width');
                }
              });
            }
          },
...
async function optimizeSvg(
  ...
  const optimizedCode = optimizedSvg.data.replace(
    /<svg (.*?)>/,
    `<svg xmlns="http://www.w3.org/2000/svg" class="aura" viewBox="0 0 512 512"><title>${svgData.title}</title>`,
  );
...
async function createSvgSymbols(version: string, distDir: string, srcSvgData: SvgData[]) {
  ...
  const lines = [
    `<svg data-auraicon="${version}" style="display:none">`,
    `<style>`,
    `.aura {`,
    `  fill: currentColor;`,
    `  stroke: currentColor;`,
...
async function createCheatsheet(
  ...
  const c = srcSvgData.map(
    (svgData) => `<a href="./assets/${svgData.fileName}"><svg><use href="#${svgData.iconName}" xlink:href="#${svgData.iconName}"/></svg></a>`,
  );
...
async function getSvgs(srcSvgDir: string, distSvgDir: string, distAuraiconsDir: string): Promise<SvgData[]> {
  const optimizedSvgDir = join(distAuraiconsDir, 'svg');
...
async function createEsmIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Auraicon v${version}, ES Modules */`, ``];
...
async function createCjsIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Auraicon v${version}, CommonJS */`, ``];
...
async function createDtsIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Auraicon v${version}, Types */`, ``];
...
async function createDataJson(version: string, srcDir: string, distDir: string, srcSvgData: SvgData[]) {
  ...
  const distDataJsonPath = join(distDir, 'auraicon.json');
